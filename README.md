...
Для оформления консольного интерфейса используется библиотека `Rich`.
Она применяется для создания панелей, таблиц, цветного вывода, форматированного ввода и сообщений об успешных и ошибочных операциях.

Основные компоненты `Rich`, используемые в проекте:

* `Console` — вывод информации в терминал;
* `Panel` — создание рамок и визуальных блоков;
* `Table` — форматированный вывод меню и данных;
* `Align` — выравнивание содержимого;
* `Group` — объединение нескольких элементов интерфейса;
* `Prompt` — удобный ввод данных пользователя.
...
Файл `.env` содержит настройки подключения к PostgreSQL и не должен добавляться в Git.

В проекте используется библиотека `python-dotenv`, которая загружает значения переменных окружения из `.env`.
Конфиденциальные данные, такие как пароль базы данных, не хранятся непосредственно в исходном коде.

Файл `.env.example` содержит пример необходимых переменных без реальных учетных данных и используется как шаблон для настройки проекта.

Пример последовательности настройки:

1. Скопировать `.env.example` в `.env`.
2. Указать свои параметры подключения к PostgreSQL.
3. Не добавлять `.env` в Git.
...
После этого необходимо заполнить значения `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `DB_HOST` и `DB_PORT`.

## Rich и консольный интерфейс

Интерфейс приложения построен с использованием `Rich`.

Все основные меню используют единый визуальный стиль: панели с рамками, таблицы для пунктов меню, цветовое оформление и форматированный ввод пользователя.

Общие элементы интерфейса вынесены в `ui/ui_helpers.py`, что позволяет повторно использовать компоненты Rich во всех меню приложения.
